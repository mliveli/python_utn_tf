...
def f_boton_disponibilidad():
    ...
    if variable_autos.get() == "Vehiculo":
        messagebox.showinfo(
        ...
        for t1 in range(len(fechas_temporales)):
            for t2 in range(entrada):
                if fechas_temporales[t1] == fechas[t2].split(" ")[0]:
                    if len(fechas[t2].split(" ")) == len(lista_de_autos):
                        flag = 1

        if flag == 0:
            if test_disponibilidad_por_auto():
                marcador_disponibilidad.configure(background="yellow")
                marcador_disponibilidad.delete(0, 100)
                marcador_disponibilidad.insert(0, "Auto ocupado")
                return
...
def test_disponibilidad_por_auto():
    fechas = cargar_lista_de_disponibilidad()
    ...
    for t1 in range(len(fechas_temporales)):
        for t2 in range(entrada):
            if fechas_temporales[t1] == fechas[t2].split(" ")[0]:
                ...
                for mismo_auto in range(len(fechas[t2].split(" "))):

                    if fechas[t2].split(" ")[mismo_auto] == variable_autos.get():
                        flag = 1
    if flag == 1:
        messagebox.showinfo(
            message="Existe disponibilidad para esta fecha, pero el vehiculo que eligio se encuentra reservado. Elija otro vehiculo.",
            title="Vehiculo Reservado",
        )
        borrar_autos_reservados()
        return True
    else:
        return False


def borrar_autos_reservados():
    ...
    for t1 in range(len(fechas_temporales)):
        for t2 in range(entrada):
            if fechas_temporales[t1] == fechas[t2].split(" ")[0]:
                for t3 in range(1, len(fechas[t2].split(" "))):

                    nueva = list()

                    for i in range(len(variable_autos["value"])):
                        print(
                            fechas[t2].split(" ")[t3],
                            "  *  ",
                            variable_autos["value"][i],
                        )
                        if not fechas[t2].split(" ")[t3] == variable_autos["value"][i]:
                            nueva.append(variable_autos["value"][i])
                    variable_autos["value"] = tuple(nueva)
                    variable_autos.current(1)
...
# ***********************************************************************
# funcion que actualiza el combobox de la lista de autos al desplegarla *
# ***********************************************************************


def actualizar_lista_de_autos():
    print("**********", variable_autos["values"])
...
variable_autos.place(x=450, y=17, height=30)
...
e_nombre = ttk.Entry(
    validate="key", validatecommand=(root.register(validarnombre), "%S")
)
...
def validarnumero(text):
    return text.isdecimal()


e_telefono = ttk.Entry(
    validate="key", validatecommand=(root.register(validarnumero), "%S")
)
e_telefono.place(x=120, y=420, width=710, height=25)

##########################################################
################ DIRECCION SIN SIGNOS ####################


def validardireccion(text):
    return text.isalnum()


e_direccion = ttk.Entry(
    validate="key", validatecommand=(root.register(validardireccion), "%S")
)
