...
reserva = []
id_datos = 0
lista_de_autos = ["Vehiculos", "Volkswagen", "Fiat", "Chevrolet"]
cont = 0
...
def test_disponibilidad():
    ...
    for i in range(dias + 1):  # revisar el +1
        f_inicio = f_inicio + timedelta(days=1)
...
def f_boton_reservar(con, tree):
    ...
    for c in variable_autos["values"]:
        ...
        print(c, "==", variable_autos.get())
        if not (c == variable_autos.get()):
            nueva.append(c)
